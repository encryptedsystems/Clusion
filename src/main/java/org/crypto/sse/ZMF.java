...
public class ZMF {
	...
	public static List<boolean[]> setupSetM(byte[] key, String keyword,  Multimap<String, String> documentsComposition,	Multimap<String, String> keywordComposition, int maxLengthOfMask, int falsePosRate) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, IOException{
			...
			for (String word : documentsComposition.get(identifier)){
				...
				for (int i=0;i<falsePosRate;i++){
					...
					if (!state.contains(position)){
						...
						//System.out.println("Time of the OCs "+ messageBol.length+ " "+(endTime-startTime)+ " "+(endTime1-startTime1));
		...
		/*
		 * The version 2 of Setup handles the OC evaluation in a much faster way where
		 * the client will store in the setup phase all possible evaluations for the largest BF
		 * and will truncate these pre-computed values when required. The first version of Setup was 
		 * computing the OC for every BF which was redundant.
		 */
		...
		public static Map<String,boolean[]> setupSetMV2(byte[] key, String keyword,  Multimap<String, String> documentsComposition,	Multimap<String, String> keywordComposition, int falsePosRate) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, IOException{
			...
			//Create a Set that contains all elements of documents that contain "keyword"
			for (String identifier : keywordComposition.get(keyword)){	

				// Filtering the keywords that are associated to a large number of keywords
				for(String word : documentsComposition.get(identifier)){	
					if (!totalElements.get(word).contains(identifier) && ((double)TextExtractPar.lp1.get(word).size()/TextExtractPar.maxTupleSize > IEXZMF.filterParameter)){
			...
			// This can be used to reduce the size of the Bloom filters to store only significant keywords
			...
			//System.out.println("Key PRF "+ CryptoPrimitives.booleanToString(CryptoPrimitives.bytesToBoolean(keyPRF)));
			...
			for (String identifier : keywordComposition.get(keyword)){
				...
				//Assuming of course that there is no filter with more than 2^1000 cells
				for (int j=0;j<1000;j++){
			...
			for (String word : totalElements.keySet()){
				...
				for (String id : totalElements.get(word)){
					...
					//determine the size of the Matryoshka filter
					int count=0;
					//double filterSize= falsePosRate * documentsComposition.get(id).size()/Math.log(2);
					double filterSize =0;
					...
					for (int j=0;j<falsePosRate;j++){
						...
						// Computation of the position where the element will be inserted
						byte[] hmac	=	CryptoPrimitives.generateHmac(keyHMAC, 
								CryptoPrimitives.concat(String.valueOf(j).getBytes(),
										CryptoPrimitives.generateHmac(keyPRF, CryptoPrimitives.concat(keyword.getBytes(),word.getBytes()))));
	...
	//***********************************************************************************************//	
	public static List<byte[]> genTokSMV2(byte[] key, String keywordONE,  String keywordTWO, int maxLengthOfMask, int falsePosRate) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, IOException{
		...
		//computation of the PRF based on CMAC-AES
		byte[] cmac=CryptoPrimitives.generateHmac(keyPRF, CryptoPrimitives.concat(keywordONE.getBytes(),keywordTWO.getBytes()));
		...
		for (int i=0;i<falsePosRate;i++){
			byte[] hmac	=	CryptoPrimitives.generateHmac(keyHMAC, 
					CryptoPrimitives.concat(String.valueOf(i).getBytes(),
							cmac));
			//We truncate the needed bits from the output of the HMAC to get the bit 1 to counter
			position	=	CryptoPrimitives.getIntFromByte(hmac, maxLengthOfMask);
	...
	public static boolean[]	testSMV2(Map<String,boolean[]> listOfbloomFilter, List<byte[]> token, int ratePos) throws UnsupportedEncodingException{
